        ...
        if($_SERVER['REQUEST_METHOD']=="POST"){
            ...
            if($title == '' || $description == '' || $contributors == '' || $dat == ''){
				...
				if (in_array($file_ext, $permited) === false) {
					...
					$query = "INSERT INTO project(title, image, description, contributors, dat) VALUES('$title', '$uploaded_image', '$description', '$contributors', '$dat')";
					$inserted_rows = $db->insert($query);
						if ($inserted_rows) {
							echo "<span style='font-size:18px;color:green'>New project is uploaded successfully.!!
							</span>";
						}else {
